using System.Threading.Tasks;
using Feree.ResultType.Errors;
using Feree.ResultType.Results;
using Feree.ResultType.Unit;

namespace Feree.ResultType.Factories
{
    public static class ResultFactory
    {
        public static IResult<T> CreateSuccess<T>(T payload) => new Success<T>(payload);
        public static IResult<Empty> CreateSuccess() => new Success<Empty>(new Empty());
        
        public static IResult<T> CreateFailure<T>(IError error) => new Failure<T>(error);
        public static IResult<T> CreateFailure<T>(string message) => new Failure<T>(new Error(message));
        public static IResult<Empty> CreateFailure(IError error) => new Failure<Empty>(error);
        public static IResult<Empty> CreateFailure(string message) => new Failure<Empty>(new Error(message));
        
        public static async Task<IResult<T>> CreateSuccessAsync<T>(Task<T> payload) => new Success<T>(await payload);
        
        public static async Task<IResult<T>> CreateFailure<T>(Task<IError> error) => new Failure<T>(await error);
        public static async Task<IResult<T>> CreateFailure<T>(Task<string> message) => new Failure<T>(new Error(await message));
        public static async Task<IResult<Empty>> CreateFailure(Task<IError> error) => new Failure<Empty>(await error);
        public static async Task<IResult<Empty>> CreateFailure(Task<string> message) => new Failure<Empty>(new Error(await message));
    }
}